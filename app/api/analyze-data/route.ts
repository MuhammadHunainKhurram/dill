...
export async function POST(req: Request) {
  try {
    ...
    // Allow non-authenticated users to use the feature, but don't store results
    ...
    // Store results in Supabase only if user is authenticated
    let storagePath: string | null = null;
    if (user) {
      try {
        const now = Date.now();
        storagePath = `${user.id}/data-analysis/${now}_analysis.json`;
        ...
        const analysisData = {
          ...
          metadata,
          analysis,
          raw_data: combinedData.slice(0, 1000) // Store first 1000 rows
        };
